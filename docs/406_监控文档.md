...
# 告警日志写入 ./logs/alertmanager_webhook.log，控制台实时打印
```
...
# 3) 端到端验证告警链路：向 Alertmanager 注入一条合成告警，观察 ./logs/alertmanager_webhook.log
...
sleep 35 && grep -c SmokeTest ./logs/alertmanager_webhook.log   # 期望 >=1
```
...
- **告警不发送**：检查 Alertmanager 容器能解析 `host.docker.internal`（`docker-compose.monitoring.yml`
  已给 alertmanager 加 `extra_hosts`）；确认 webhook 接收器在 `:9095` 运行；看 `./logs/alertmanager_webhook.log`。
