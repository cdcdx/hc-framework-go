...
> 当前形态:**rest 网关 + 单一领域 rpc(hc-rpc)**(2 进程,由早期 4-rpc 架构合并而来,见 904 变更记录)。
> 本文说明:如何生成缺失代码并跑起来、目录与契约设计、与原版差异与取舍。

## 1. 架构总览

```
   HTTP :8080 ────────► gateway-api (rest)
                         │  JwtAuth / SecurityIPLimit / Captcha / 统一响应
                         │  gRPC(zrpc, etcd 服务发现)
                         ▼
                      hc-rpc (:8001)   ← 单一领域服务
                         ├─ user 域  认证/用户/积分
...
- 网关持有 1 个 `HcRpc` 客户端,做参数绑定、鉴权、验证码、IP 限频、统一响应。
- 域间协作(积分增减、任务进度)为**进程内调用**,无跨服务调用。
- 服务发现用 etcd;无 etcd 时 `HcRpc.Target: direct://127.0.0.1:8001` 直连(见 902 §3)。

## 2. 代码生成与启动(在有 go 工具链的机器上执行)

本仓库已由 goctl/protoc 生成过 4-rpc 时代的 pb 代码并跑通编译;
合并为单 rpc 后需**重新生成一次**(`app/rpc/hc/hc.pb.go`、`hc_grpc.pb.go`):
...
|---|---|---|
| 进程模型 | 单进程单体 | 网关 + 单一 rpc,共 2 进程 |
| 服务间调用 | 同进程函数调用 | 网关→rpc 走 gRPC;rpc 内域间为进程内调用 |
...
4. **sqlite 单文件**:`hc-rpc` 默认 `data/hc.db`;生产切 mysql/postgres 改 `etc/hc.yaml` 的
   `DB.Driver/Dsn` 即可(表结构未变,migrations/ 可直接复用)。
