...
- [03_架构说明.md](./03_架构说明.md) §6.4 — 异步事件与协调级联（选主 / 死分片接管）。
- [09_可观测性与告警.md](./09_可观测性与告警.md) — `idle_scan_*` / `idle_settle_total` 指标清单与告警规则。
- [05_配置参考.md](./05_配置参考.md) §2.2 / §11.2 — `cache.l2.*` 与 `idle.*` 全量字段。
- [07_集群设备部署手册.md](./07_集群设备部署手册.md) §4.1 / §11 — Redis Cluster 物理拓扑与 K8s 清单。
