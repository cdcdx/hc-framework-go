...
type ShopItem struct {
	...
	SalesStatus string    `json:"sales_status" gorm:"-"` // 派生销售状态（不落库，仅供展示）
...
// SalesStatusValue 根据权威字段（IsActive / Stock）派生销售状态。
// 仅用于展示；库存扣减与防超卖由 DeductStock 原子条件更新兜底。
...
// 先到先得；每个用户最多抢 PerUserLimit 个。抢购限量由本表 SoldQty/LimitQty 独立管理，
// 与 ShopItem.Stock 解耦。
//
// 防超卖兜底：DB 条件更新（UPDATE ... SET sold_qty=sold_qty+1 WHERE id=? AND sold_qty<limit_qty），
...
// SalesStatusValue 根据权威字段（Status / StartTime / EndTime / SoldQty / LimitQty）与时间窗派生销售状态。
// 仅用于展示；抢购配额扣减与防超卖由 AcquireFlashQuota 原子条件更新兜底。
