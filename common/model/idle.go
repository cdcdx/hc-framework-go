...
// 挂机记录状态
const (
	IdleStatusActive    = "active"    // 挂机中
	IdleStatusCompleted = "completed" // 正常结束
	IdleStatusTimeout   = "timeout"   // 心跳超时被结算
)
...
// IdleDailyPoints 每日挂机积分汇总表。
// 每次结算在业务事务内增量累加 (user_id, day) 的 total；GetDailyPoints 的 DB 回源读单行 O(1)。
